...
void Token_stream::putback(Token t)  // Сохранение токена в буфер
{
    if (full)
...
Token Token_stream::get()  // Чтение символа с потока входа
{
    if (full)  // Чтение с буфера
    {
        full = false;
        return buffer;
    }
    ...
    cin >> ch;
    if(!cin){
        std::cout << "Работа с калькулятором окончена ";
        exit(0);
    }
    switch (ch)
    {
