...
void Token_stream::putback(Token t) //reserves a token
{
    if (full)
...
Token Token_stream::get() //gets a token off the input stream
{
    if (full) //gets value out of a buffer if it is full
    {
        full = false;
        return buffer;
    }
    ...
    char ch = ' ';
    while(isspace(ch)) //removes all space characters
    {
        ch = cin.get();
        if(cin.eof())
        {
