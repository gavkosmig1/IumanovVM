...
double Symbol_table::get_value(
    const string& var_name)  // Получение значения переменной
{
...
bool Symbol_table::is_declared(
    const string& var_name)  // Занято ли имя переменной чек
{
  for (size_t i = 0; i < Symbol_table::v.size(); ++i)
  {
    if (Symbol_table::v[i].name == var_name)
...
double Symbol_table::set_value(const string& s, double d)  //
{
  for (size_t i = 0; i < Symbol_table::v.size(); ++i)
  {
    if ((Symbol_table::v[i].name == s) &&
        (!(Symbol_table::v[i].isconstant)))
    {
